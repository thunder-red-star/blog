...
const apiFiles = fs
	.readdirSync("api")
	.filter((file) => file.split(".")[file.split(".").length - 1] === "js");
	...
	if (data.get) {
		try {
			let endpointName = endpoint
				.split(".")
				.slice(0, endpoint.split(".").length - 1)
				.join(".");
			app.get("/api/" + endpointName, (req, res) => {
				return data.get(req, res);
			});
			term
				.green("[INFO] GET endpoint added: ")
				.bold.brightGreen(endpointName + "\n");
		} catch (err) {
			term
				.red("[WARN] GET endpoint addition failed: ")
				.bold.red(endpointName + "\n");
			term.red("[WARN] " + err + "\n");
		}
	}
	if (data.post) {
		try {
			...
			term
				.green("[INFO] POST endpoint added: ")
				.bold.brightGreen(endpointName + "\n");
		} catch (err) {
			term
				.red("[WARN] POST endpoint addition failed: ")
				.bold.red(endpointName + "\n");
			term.red("[WARN] " + err + "\n");
		}
	}
